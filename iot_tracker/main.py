    ...
    if activate_cmp_detector == 1:
        ...
        if loss_signal <= 3:
            ...
            if len(blocks_info) == 1:
                if blocks_info[0] != [0, 0, 0, 0] and temp_iou[0] > 0.1 and refind_temp[0] > 0.3:
...
def open():
    ...
    frame_img = Image.fromarray(frame_PIL)
    ...
    if len(search_prob_list)>0:
        print("draw block_info",blocks_info)
        print("draw search_prob_list:",search_prob_list)
        for k in range(len(search_prob_list)):
            person_id = search_prob_list[k]
            # draw pre_block
            if len(fade_color) > k:
                search_mask_color = fade_color[k]
            else:
                search_mask_color = fade_color[-1]
            mask = Image.new("RGB", (blocks_info[person_id][2], blocks_info[person_id][3]),
                             color=(search_mask_color))
            selected_person_img = frame_img.crop(((blocks_info[person_id][0], blocks_info[person_id][1],
                        (blocks_info[person_id][0] + blocks_info[person_id][2]),
                        (blocks_info[person_id][1] + blocks_info[person_id][3]))))
            blend_img = Image.blend(mask, selected_person_img, 0.5)  # img1*(1-0.5),img*0.5
            frame_img.paste(blend_img, (blocks_info[person_id][0], blocks_info[person_id][1]))
